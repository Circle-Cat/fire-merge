...
const TEST_INVALID_DIR = '/mock/workspace2';
...
describe('findRulesFiles', () => {
  ...
  test('there is an invalid dir path then throw error', () => {
    const MOCK_FILE_INFO: Array<{ filePath: string; content: string; type: string }> = [
      { filePath: TEST_ROOT_TEMPLATE_PATH, content: TEST_ROOT_TEMPLATE_FILE_NO_RULES_CONTENT, type: FileType.File },
    ];
    fs.__setMockFiles(MOCK_FILE_INFO);

    expect(() => findRulesFiles(TEST_INVALID_DIR, TEST_ROOT_SR_TEMPLATE)).toThrow(ErrorMessageStatic.DIR_NOT_EXIST.replace('{dir}', TEST_INVALID_DIR));
  })
