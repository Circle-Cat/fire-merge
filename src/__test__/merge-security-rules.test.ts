...
describe('Test mergeSecurityRules', () => {
  ...
  test('should merge security rules correctly and template does not have any rules', async () => {
    // 模拟 findRulesFiles 返回值
    findRulesFilesSpy.mockReturnValue({
      rootSrTemplateFile: TEST_ROOT_TEMPLATE_PATH,
      subSrFiles: [TEST_FIRST_RULES_FILE, TEST_SECOND_RULES_FILE],
    });

    // 模拟 readFileContent 返回值
    readFileContentSpy
      .mockResolvedValueOnce(TEST_FIRST_RULES_FILE_CONTENT)
      .mockResolvedValueOnce(TEST_SECOND_RULES_FILE_CONTENT)
      .mockResolvedValueOnce(TEST_ROOT_TEMPLATE_FILE_NO_RULES_CONTENT);

    // 执行 mergeSecurityRules 函数
    await mergeSecurityRules.mergeSecurityRules(TEST_DIR, TEST_ROOT_SR_TEMPLATE, TEST_OUT_PUT_FILE);

    // 验证 findRulesFiles 是否被调用
    expect(findRulesFilesSpy).toHaveBeenCalledWith(TEST_DIR, TEST_ROOT_SR_TEMPLATE);
    expect(findRulesFilesSpy).toHaveBeenCalledTimes(1);

    // 验证 readFileContent 是否被调用
    expect(readFileContentSpy).toHaveBeenCalledTimes(3);
    expect(readFileContentSpy).toHaveBeenCalledWith(TEST_ROOT_TEMPLATE_PATH);
    expect(readFileContentSpy).toHaveBeenCalledWith(TEST_FIRST_RULES_FILE);
    expect(readFileContentSpy).toHaveBeenCalledWith(TEST_SECOND_RULES_FILE);
  ...
  test('should merge security rules correctly and template contain some rules', async () => {
    ...
    readFileContentSpy
      .mockResolvedValueOnce(TEST_FIRST_RULES_FILE_CONTENT)
      .mockResolvedValueOnce(TEST_SECOND_RULES_FILE_CONTENT)
      .mockResolvedValueOnce(TEST_ROOT_TEMPLATE_FILE_HAS_RULES_CONTENT);
