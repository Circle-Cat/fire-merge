...
# Test 5: Invalid dir path (should throw DIR_NOT_EXIST)
run_test "Test 5: Invalid dir path" "./src/e2e-tests/workspace-error" "DIR_NOT_EXIST"

# Test 6: Overwrite existing firestore.rules
WORKSPACE_PATH="./src/e2e-tests/workspace-success-firestore-existed"
echo "Running Test 6: Overwrite Existing firestore.rules"
...
if grep -q "// OLD CONTENT, SHOULD BE OVERWRITTEN" "$WORKSPACE_PATH/$OUTPUT_FILE"; then
  echo "Test 6 Failed (firestore.rules was not properly overwritten)"
else
  echo "Test 6 Passed (firestore.rules successfully overwritten)"
fi
